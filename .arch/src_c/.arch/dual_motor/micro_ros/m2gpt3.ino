...
// PWM parameters (new LEDC API does not use a separate channel parameter)
...
void set_motor_speed(uint8_t in1, uint8_t in2, uint8_t pwm_pin, float speed) {
  ...
  // Write PWM duty using the new API (pass the pin, not channel)
...
void cmd_vel_callback(const void *msgin) {
  ...
  // Set motor speeds accordingly (pass PWM pin instead of channel)
...
void timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
  ...
  float dt = (current_time - last_rpm_time) / 1000.0; // seconds
...
void setup() {
  ...
  // Note: ledcAttach now takes only (pin, frequency, resolution)
  ...
  // Initialize executor with capacity for subscription and timer handles
